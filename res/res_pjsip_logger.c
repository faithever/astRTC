...
static enum pjsip_logger_mask logger_cli_settings = AST_PJSIP_LOGGER_ALL;//ymlee:change log to LOGGER_ALL//AST_PJSIP_LOGGER_UNSET;
...
static void check_debug(void)
{
	...
	ast_debug(5, "########## Debug mode %s:%d\n", debug, ast_true(debug));
	if (ast_true(debug)) {
...
static int load_module(void)
{
	...
	check_debug();
	pjsip_enable_logger_all(1); //ymlee:add:enable log
