...
static void log_forwarder(int level, const char *data, int len)
{
   ...
   //printf("########### ast_level: %d and level: %d, __LOG_SUPPRESS: %d\n", ast_level, level, __LOG_SUPPRESS);
	 ...
	 * up */
   //ymlee:change:From ast_level to __LOG_VERBOSE
	//ast_log(ast_level, log_source, log_line, log_func, "\t%s\n", data);
	ast_log(__LOG_VERBOSE, log_source, log_line, log_func, "\t%s\n", data);
}
